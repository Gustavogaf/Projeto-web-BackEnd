...
public class EsporteControllerTest {
    ...
    public void deveCriarUmEsporteComSucesso() throws Exception {
        ...
        ResultActions resultado = mockMvc.perform(post("/api/esportes")
                ...
                .content(objectMapper.writeValueAsString(esporteParaEnviar))); // Convertemos nosso objeto para JSON

        // 3. VERIFICAÇÃO
        // Verificamos se a resposta da API é a esperada
        resultado.andExpect(status().isCreated()) // Esperamos o status HTTP 201 Created
                 .andExpect(jsonPath("$.id").value(1L)) // Esperamos que o JSON de resposta tenha um campo "id" com valor 1
                 .andExpect(jsonPath("$.nome").value("Natação")); // E um campo "nome" com valor "Natação"
    }
