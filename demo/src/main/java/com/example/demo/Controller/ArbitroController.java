...
public class ArbitroController {
    ...
    @PutMapping("/{matriculaArbitro}/partidas/{partidaId}/reverter")
    public ResponseEntity<?> reverterResultado(
            @PathVariable String matriculaArbitro,
            @PathVariable Long partidaId) {

        try {
            Partida partidaRevertida = arbitroService.reverterResultado(matriculaArbitro, partidaId);
            return new ResponseEntity<>(partidaRevertida, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
        }
    }
